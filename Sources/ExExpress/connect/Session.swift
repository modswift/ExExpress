...
fileprivate func nextSessionID(msg: IncomingMessage) -> String {
            ...
            -> Middleware
{
  return { req, res, next in
    ...
    // retrieve from store
    do {
      try s.get(sessionID: sessionID) { session in
    ...
    catch let err as SessionStoreError where err == .SessionNotFound {
...
fileprivate class SessionContext {
...
public protocol SessionStore {
  ...
  func get(sessionID sid: String, _ cb: ( Session? ) throws -> Void) throws
